...
def train_transforms(img_height, img_width):
    return A.Compose(
        [A.Resize(height=img_height, width=img_width, p=1.0), ToTensorV2(p=1.0)],
        p=1.0,
...
def run():
    ...
    train_dataset = dataset.DetectionDataset(
        train_df,
        config.IMAGE_DIR,
        config.TARGET,
    ...
    valid_dataset = dataset.DetectionDataset(
        valid_df,
        config.IMAGE_DIR,
        config.TARGET,
        transforms=train_transforms(config.IMG_HEIGHT, config.IMG_WIDTH),
    ...
    print(
        """Model Created with backbone = {}, pretrained = {}, 
        number of classes = {}, number of queries = {}""".format(
            config.BACKBONE, config.PRETRAINED, config.NUM_CLASSES, config.NUM_QUERIES
        )
    )
    ...
    criterion = detr_loss.SetCriterion(
    ...
    for epoch in tqdm(range(config.EPOCHS)):
        ...
        train_loss = engine.train_fn(
            train_dataloader, detector, criterion, optimizer, device
        )
